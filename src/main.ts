...
class Game implements IGame {
  ...
  private async determinePlayerOrder(): Promise<[Player, Player]> {
    ...
    console.log(
      `${
        firstPlayer.getName() === "Computer" ? "I" : "You"
      } make the first move.`
    );
  ...
  private async chooseDice(
    ...
    if (firstPlayer.getName() === "Computer") {
      ...
      console.log(
        `I make the first move and choose the [${dice[
          computerChoice
        ].getValues()}] dice.`
      );
      ...
      console.log(
        `You choose the [${secondPlayer.getDice()?.getValues()}] dice.`
      );
    } else {
      console.log("Choose your dice:");
      dice.forEach((die, i) => {
      ...
      console.log(
        `You choose the [${firstPlayer.getDice()?.getValues()}] dice.`
      );
      ...
      console.log(
        `I choose the [${secondPlayer.getDice()?.getValues()}] dice.`
      );
    }
  }
  ...
  private async playDiceRoll(
    ...
    if (firstPlayer.getName() === "Computer") {
      ...
      if (this.verifier.verifyHmac(computerNumber, key, computerHmac)) {
        console.log(
          `My number is ${computerNumber} (KEY=${key.toString("hex")}).`
        );
        const result = (computerNumber + playerChoice) % 6;
        console.log(
          `The fair number generation result is ${computerNumber} + ${playerChoice} = ${result} (mod 6).`
        );
        ...
        if (this.verifier.verifyHmac(playerNumber, playerKey, playerHmac)) {
          console.log(
            `My number is ${playerNumber} (KEY=${playerKey.toString("hex")}).`
          );
          const playerResult = (playerNumber + playerSecondChoice) % 6;
          console.log(
            `The fair number generation result is ${playerNumber} + ${playerSecondChoice} = ${playerResult} (mod 6).`
          );
      ...
      if (this.verifier.verifyHmac(playerNumber, playerKey, playerHmac)) {
        ...
        console.log(
          `The fair number generation result is ${playerNumber} + ${playerChoice} = ${result} (mod 6).`
        );
        ...
        if (
          this.verifier.verifyHmac(computerNumber, computerKey, computerHmac)
        ) {
          console.log(
            `My number is ${computerNumber} (KEY=${computerKey.toString(
              "hex"
            )}).`
          );
          const computerResult = (computerNumber + computerChoice) % 6;
          console.log(
            `The fair number generation result is ${computerNumber} + ${computerChoice} = ${computerResult} (mod 6).`
          );
